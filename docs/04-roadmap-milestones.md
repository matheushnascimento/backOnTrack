...
- 🟡 Identidade visual do "Back on Track" (tema, ícones) — migrada do QoL. **Parte A entregue** (#181): `app.json` `name` vira "Back on Track" (display), `splash.backgroundColor` alinha com a paleta (`#F8F9FA`), wordmark discreto no rodapé da Home, e `docs/08-design-tokens.md` ganha seção "Identidade visual" com nome/slug/tagline/metáfora/tom de voz/paleta como marca. **Parte B pendente:** os 4 assets (`icon.png`, `adaptive-icon.png`, `splash-icon.png`, `favicon.png`) ainda são template default do Expo — requer design real, follow-up rastreado como card no ambiente
...
- 🟡 Escolher e configurar o Synchronizer do TinyBase. **Escolha feita** na ADR-009 (#192): WebSocket próprio no homeserver via `createWsSynchronizer` + `createWsServer` (Node/Docker), cliente migra `Store` → `MergeableStore`. Alternativas (PowerSync, Yjs, só backup, Cloudflare Durable Objects) descartadas com motivo. **Configurar** em 3 fatias: **fatia 1** cliente migrado pra `MergeableStore` (#195) sem quebrar comportamento (persisters em JSON mode); **fatia 2** server WS em Docker no homeserver com `createWsServer` + persister JSON por sala + smoke test round-trip local + testes automatizados (subprocess, cobre bugs de código sem depender de rede) + deploy via Cloudflare Tunnel `home server` existente (rota `backontrack-sync.mhdn.com.br` → `127.0.0.1:8787`, TLS público confirmado com smoke remoto) (#198). **Falta:** fatia 3 — cliente do app conecta no server via `createWsSynchronizer` (integração em `infra/persistence.js`)
