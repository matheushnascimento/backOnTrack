...
- 🟡 Autenticação/identificação do usuário. **Decisão feita** na ADR-010 (#205): Supabase Auth hospedado, magic link por email. Cliente ganha telas de login/logout + `SessionProvider`; server WS valida JWT no upgrade e deriva `roomId = user.id`; dados anônimos migram no 1º sign-in. Substitui o roomId aleatório por-install da fatia 3 do sync. **Fatiar em:** **A** setup Supabase + telas de login (sync continua anônimo); **B** server valida JWT + rooms por userId (breaking change pros testers — precisa aviso via WhatsApp antes do rollout); **C** migração dos dados anônimos existentes pra sala do userId
