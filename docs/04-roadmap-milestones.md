...
- 🟡 Componentizar o que está duplicado (`CARD`/`LABEL` repetidos em 3 telas → componente; padronizar `MyButton`/`HistoryCard`/`MyHeader`). Fatia 1 feita (#169): `Card`, `SectionLabel` e `FieldLabel` extraídos em `components/`, com 6 telas topo-nível e 3 arquivos de métrica migrados; constantes `CARD`/`LABEL`/`FIELD_LABEL` duplicadas apagadas. Fatia 2 feita (#172): `MyButton`/`MyIconButton` ganham contrato `disabled` canônico (aplica `opacity-40` internamente, `Pressable` recebe `disabled` explícito), 3 callsites (history/admin/feedback) simplificam. Falta: padronização da largura do card do form de métrica (follow-up card, cards nested do `fields.jsx`/`registry.jsx`)
